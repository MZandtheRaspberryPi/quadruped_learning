...
import retarget_config_bittle_petoi as config
from motion_retargeting import motion_util
import pybullet_data as pd
import pybullet
from pybullet_utils import transformations
from motion_retargeting import pose3d
import numpy as np
...
def set_pose(robot, pose):
    ...
    for j in range(num_joints):
        ...
        if (j_pose_size > 0):
            ...
            print(f"pybullet.resetJointStateMultiDof({robot}, {j}, {j_pose}, {j_vel})")
...
def retarget_motion(robot, joint_pos_data):
    ...
    for f in range(num_frames):
        ...
        curr_pose = retarget_pose(
            robot, config.DEFAULT_JOINT_POSE, ref_joint_pos)
...
def main():
    ...
    while True:
        ...
        for mocap_motion in mocap_motions:
            ...
            pose = np.array([ 0.        ,  0.        ,  0.43701   , -0.00543505,  0.04502121,
        0.04979647,  0.99772935, -0.82915742,  2.17664924,  0.36769588,
        1.27266405, -1.42078914,  2.89240592,  0.8235345 ,  2.13203177])
            set_pose(robot, pose)
